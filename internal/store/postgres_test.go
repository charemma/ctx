...
func testDatabaseURL(t *testing.T) string {
	t.Helper()
	url := os.Getenv("CTX_TEST_DATABASE_URL")
	if url == "" {
		url = "postgres://ctx:ctx@localhost:5432/ctx?sslmode=disable"
	}
	return url
}
...
func setupStore(t *testing.T) *PostgresStore {
	...
	s, err := NewPostgresStore(ctx, testDatabaseURL(t))
	if err != nil {
...
func TestDocumentUpsertAndQuery(t *testing.T) {
	s := setupStore(t)
	ctx := context.Background()
...
func TestChunksCRUD(t *testing.T) {
	s := setupStore(t)
	ctx := context.Background()
	...
	// Need the actual document ID from the DB.
